...
// component
function Sidebar() {
	const [scrollingDown, setScrollingDown] = useState(false); // State for detecting scroll direction

	useEffect(() => {
		const handleScroll = () => {
			const scrollTop = sidebarRef.current.scrollTop;
			const isScrollingDown = scrollTop > 0; // Detect if user is scrolling down
	...
	return (
		<Box
			sx={{
				...
				transition:
					"width 0.5s ease-in-out, backdrop-filter 0.5s ease-in-out", // Smooth transition effect
			}}
		>
			<Box
				ref={sidebarRef}
				sx={{
					...
					boxShadow: scrollingDown
						? "inset 0px 10px 20px rgba(94, 85, 54, 0.5)"
						: "inset 0px -10px 20px rgba(94, 85, 54, 0.5)",
					...
					"&::-webkit-scrollbar-thumb": {
						backgroundColor: "#5e5536", // Custom scrollbar thumb color
						borderRadius: "10px",
					},
